...
| 7 | Real Integrations | MessageBird SMS, Web Push, Google Maps travel/geofence, Gemini feedback classification |
...
On the auth side: wire Supabase Auth's phone-OTP sign-up flow for customers (App Flow section 5, steps 1–3 only — name/phone, OTP, password; avatar and notification preferences are Phase 3) and email/phone-plus-password login for staff (App Flow section 8.2). A distinction worth being explicit about here: OTP-for-login is Supabase Auth's own core mechanism, not part of the notification system this plan stubs until Phase 7 — it needs a working SMS provider (MessageBird, configured directly in the Supabase project's Auth settings, since MessageBird is one of Auth's natively-supported phone-OTP providers and needs no custom code for this path) from this phase onward, or Supabase's local/staging test-OTP mode if that's available for the staging project tier, since a customer literally cannot sign up without it.
